...
class Program
{
    ...
    static List<(int cost, int to)>[] adj = new List<(int, int)>[10005];
    static bool[] chk = new bool[10005]; // chk[i]: i번 정점이 MST에 포함됐는가
    ...
    static void Main()
    {
        ...
        // 최소 힙: (비용, 정점 a, 정점 b)
        var pq = new PriorityQueue<(int cost, int a, int b), int>();

        chk[1] = true;
        foreach (var nxt in adj[1])
            pq.Enqueue((nxt.cost, 1, nxt.to), nxt.cost);

        while (cnt < v - 1)
        {
            var (cost, a, b) = pq.Dequeue();
            if (chk[b]) continue;
            ...
            foreach (var nxt in adj[b])
            {
                if (!chk[nxt.to])
                    pq.Enqueue((nxt.cost, b, nxt.to), nxt.cost);
            }
        }
